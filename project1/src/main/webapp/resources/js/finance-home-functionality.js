/**
 * File that implements the functionality in the finance manager home page
 */
...
window.onload = function(){
    ...
    document
        .getElementById('allReimbs')
        .addEventListener('click', getAllReimbursements);
        
    document
        .getElementById('pendingReimbs')
        .addEventListener('click', getAllPendingReimbursements);
    
    document
        .getElementById('disapproveReimbs')
        .addEventListener('click', disapproveReimbursement);

    document
        .getElementById('approveReimbs')
...
function getUserName(){
 ...
 //Gets every reimbursement in the system
 function getAllReimbursements(){
	fetch('http://localhost:9001/project1/api/ajax/everyReimbursement')
        .then(
            function(daResponse){
                const convertedResponse = daResponse.json();
                return convertedResponse;
...
//Gets every reimbursement in the system that is pending
function getAllPendingReimbursements(){
    fetch('http://localhost:9001/project1/api/ajax/pendingReimbursements')
        .then(
            function(daResponse){
                const convertedResponse = daResponse.json();
                return convertedResponse;
...
//Changes the status of a reimbursement to denied
function disapproveReimbursement(){
    let disReimbID = document.getElementById('approveDenyID').value;
    
    fetch(`http://localhost:9001/project1/api/ajax/disapproveReimbursement?reimbID=${disReimbID}`,
						{"method": "post"})
}

//Changes the status of a reimbursement to approved
function approveReimbursement(){
    let appReimbID = document.getElementById('approveDenyID').value;
    
    fetch(`http://localhost:9001/project1/api/ajax/approveReimbursement?reimbID=${appReimbID}`,
						{"method": "post"})
}
...
function tableDOMManipulation(ourJSON){
	...
	for (let i = 0; i < ourJSON.length; i++) {
	 ...
	 // ///////////CREATE ELEMENTS DYNAMICALLY//////////////
		...
		// population creations
        newTH.setAttribute("scope", "row")
        let myText0 = document.createTextNode(ourJSON[i].reimbID);
        let myText1 = document.createTextNode(mapForNames[ourJSON[i].reimbAuthor]);
